...
static void ow_port_io_set_input(void)
{
	 ...
	 * high. */
	ONEWIRE_PORT_IO_CONFIG_DDR_INPUT &= ~(1 << ONEWIRE_PORT_IO_CONFIG_PINNR_INPUT);
	ONEWIRE_PORT_IO_CONFIG_DDR_OUTPUT &= ~(1 << ONEWIRE_PORT_IO_CONFIG_PINNR_OUTPUT);
}
...
{
	if(bit) {
		ONEWIRE_PORT_IO_CONFIG_PORT_OUTPUT |= (1 << ONEWIRE_PORT_IO_CONFIG_PINNR_OUTPUT);
	} else {
		ONEWIRE_PORT_IO_CONFIG_PORT_OUTPUT &= ~(1 << ONEWIRE_PORT_IO_CONFIG_PINNR_OUTPUT);
	}
}
static uint8_t ow_port_io_read(void)
{
	return (ONEWIRE_PORT_IO_CONFIG_PIN_INPUT & (1 << ONEWIRE_PORT_IO_CONFIG_PINNR_INPUT));
}
